...
config = {
    "save_weights": [os.path.join(parent_dir, "weights/ber/dataset-p1-3m/")],
    "plots_dir": [os.path.join(parent_dir, "plots/mean_varience/dataset-p1-3m/")]
}
...
if __name__ == "__main__":
    ...
    for config in configurations:
        ...
        source = torch.tensor(batch_a_to_b)
        target = torch.tensor(adata2.X)

        print(eval_mmd(target,target))
        print(eval_mmd(source,target))
        adata_target_calibrated_src.write(os.path.join(config["plots_dir"],'mean_var_dataset_p1_3m.h5ad'))
